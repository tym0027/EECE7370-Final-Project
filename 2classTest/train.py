...
ckpt_file = '/storage/truppr/models/2classExperiment/UCFModel24.pth'
...
## Load input data
# set train list name:
trainFolderFile = 'trainlist02.txt'
...
for epoch in range(start_epoch, EPOCH+1):
	...
	for i_batch, sample in enumerate(dataloader):
		...
		im = inputData[:,:,3,:].squeeze(0).view(3, 240, 320).cpu();
		print(im.shape)
		print(type(im))
		a = F.to_pil_image(im)
		plt.imshow(a)
		...
		inputData = baseDyan.forward2(inputData.squeeze(0))
		...
		print("\nNetwork Expecting -- " + str(expectedOut.item()))
		print("Network returned output -- " + str(np.argmax(output.data.cpu().numpy())))
