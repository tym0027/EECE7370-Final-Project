...
class unfrozen_DyanC(nn.Module):
	...
	def forward(self, x):
		...
		x = x.view(x.shape[0],161,240,320).unsqueeze(1)
		# print("Xe re-shape: ", x.shape)	
		# Downsize latent space
		
		# x_ = x.view(161, 3, 240, 320);
		x_ = x.squeeze().cpu()
		visualization = torch.zeros(3, 16 * 240, 10 * 320)
		for row in range(0, 16):	
			for col in range(0, 10):	
				visualization[:, row * 240 : (row * 240) + 240, col * 320 : (col * 320) + 320] = x_[:, 10 * row + col,:, :] # PGOF
		print(type(visualization))
		a = tv.to_pil_image(visualization * 4)
		plt.imshow(a)
		plt.show()
		input()
