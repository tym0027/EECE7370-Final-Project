...
def im_tensor_to_numpy(x):
    transpose = transforms.ToPILImage().cuda(gpu_id)
    x = np.asarray(transpose(x)).cuda(gpu_id)
    return x
...
def save_im_tensor(x, addr):
    x = x.cpu().float()
    transpose = transforms.ToPILImage().cuda(gpu_id)
    x = transpose(x[0]).cuda(gpu_id)
    x.save(addr).cuda(gpu_id)
...
def meshgrid(height, width, n_repeat):
    ...
    grid = torch.cat([x_t_flat, y_t_flat])[None, ...].cuda(gpu_id).view(-1)
