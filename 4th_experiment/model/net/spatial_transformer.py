...
class spatial_transformer(nn.Module):
    ...
    def _interpolate(self, im, x, y, out_size):
        ...
        idx_a = (base_y0 + x0).cuda(gpu_id)
        idx_b = (base_y1 + x0).cuda(gpu_id)
        idx_c = (base_y0 + x1).cuda(gpu_id)
        idx_d = (base_y1 + x1).cuda(gpu_id)

        im = im.permute(0, 2, 3, 1)
        im_flat = im.view(-1, channels).cuda(gpu_id).float()
