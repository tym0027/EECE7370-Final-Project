...
import numpy as np
from tvnet_utils import gpu_id
...
class spatial_transformer(nn.Module):
    ...
    def _transform(self, theta, input_dim, out_size):
        ...
        theta = theta.float().cuda(gpu_id)
        
        out_height = out_size[0]
        out_width = out_size[1]
        grid = self._meshgrid(out_height, out_width).cuda(gpu_id)
    ...
    def _meshgrid(self, height, width):
        ...
        x_t = torch.matmul(torch.ones(height, 1), 
                           torch.transpose(torch.linspace(-1.0, 1.0, width)[:, None], 1, 0)).cuda(gpu_id)
        y_t = torch.matmul(torch.linspace(-1.0, 1.0, height)[:, None], torch.ones(1, width)).cuda(gpu_id)

        x_t_flat = x_t.view(1, -1).cuda(gpu_id)
        y_t_flat = y_t.view(1, -1).cuda(gpu_id)

        grid = torch.cat([x_t_flat, y_t_flat], dim=0).cuda(gpu_id)
        return grid
    ...
    def _interpolate(self, im, x, y, out_size):
        ...
        Ia = torch.index_select(im_flat, dim=0, index=Variable(idx_a.type(torch.LongTensor))) # .type(torch.cuda(3).FloatTensor)
        Ib = torch.index_select(im_flat, dim=0, index=Variable(idx_b.type(torch.LongTensor))) # .type(torch.cuda(3).FloatTensor)
        Ic = torch.index_select(im_flat, dim=0, index=Variable(idx_c.type(torch.LongTensor))) # .type(torch.cuda(3).FloatTensor)
        Id = torch.index_select(im_flat, dim=0, index=Variable(idx_d.type(torch.LongTensor))) # .type(torch.cuda(3).FloatTensor)
        ...
        A = wa * Ia.cuda(gpu_id).type(torch.cuda.FloatTensor).data
        B = wb * Ib.cuda(gpu_id).type(torch.cuda.FloatTensor).data
        C = wc * Ic.cuda(gpu_id).type(torch.cuda.FloatTensor).data
        D = wd * Id.cuda(gpu_id).type(torch.cuda.FloatTensor).data
    ...
    def _repeat(self, x, n_repeats):
        ...
        x = torch.matmul(x.view(-1, 1), rep).cuda(gpu_id)
        return x.view(-1)
